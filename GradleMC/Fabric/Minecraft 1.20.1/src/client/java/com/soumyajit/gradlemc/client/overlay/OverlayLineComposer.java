...
/** Pure title/current/average FPS composition; an empty result means no overlay residue. */
public final class OverlayLineComposer {
    ...
    public static List<String> compose(boolean compact, boolean showTitle, boolean showFps, boolean showAverageFps,
        ...
        if (compact) {
            ...
            if (showTitle) parts.add("GradleMC");
            if (showFps) parts.add(whole(fps.currentFps()) + " FPS");
            if (showAverageFps) parts.add("avg " + whole(fps.averageFps()));
            if (!parts.isEmpty()) lines.add(String.join(" | ", parts));
            return List.copyOf(lines);
        }
        if (showTitle) lines.add("GradleMC");
        if (showFps) lines.add("FPS: " + whole(fps.currentFps()));
        if (showAverageFps) lines.add("Average FPS: " + whole(fps.averageFps()));
        return List.copyOf(lines);
    }
