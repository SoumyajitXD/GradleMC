...
public final class FoundationService {
    ...
    /** Read-only multi-root planning seam.  It never captures live state or executes a task. */
    public static synchronized TaskCore.Plan plan(List<TaskId> ids, TaskCore.Context context) { return engine.plan(ids, context, false); }
    ...
    /** Executes only registered roots through the single Foundation engine. */
    public static List<TaskCore.TaskResult> run(List<TaskId> ids, TaskCore.Context context, boolean force) {
        FoundationEngine current; synchronized (FoundationService.class) { current = engine; }
        return current.execute(List.copyOf(ids), context, force);
    }
    ...
    /**
     * Captures only the immutable Foundation context on the current owning game thread.  Callers
     * must invoke this from that thread, then may pass the returned value to a worker; it contains
     * no Minecraft object.
     */
    public static TaskCore.Context captureLiveContext(Clock clock) {
        return context(clock == null ? Clock.systemUTC() : clock);
    }
    ...
    private static void registerScanV1Tasks() {
        ...
        REGISTRY.register(new TaskCore.Definition(TaskId.of("instance:snapshot"), "Immutable instance snapshot", "instance", List.of(TaskId.of("instance:identity"),TaskId.of("mods:inventory"),TaskId.of("mods:dependencies"),TaskId.of("packs:resource_inventory"),TaskId.of("packs:datapack_inventory"),TaskId.of("configs:inventory"),TaskId.of("jvm:environment"),TaskId.of("memory:snapshot")), TaskCore.Affinity.SERVER_MAIN_THREAD_CAPTURE, Duration.ofSeconds(3), TaskCore.CachePolicy.STATIC_FINGERPRINT, c->TaskCore.Availability.present(), c->output("instance:snapshot","Minecraft instance snapshot","Captured immutable bounded summaries; unavailable components remain explicit.",capture(),Map.of())));
        REGISTRY.register(new TaskCore.Definition(TaskId.of("scan:standard"), "Write GradleMC Scan v1", "scan", List.of(TaskId.of("instance:snapshot")), TaskCore.Affinity.WORKER_SAFE, Duration.ofSeconds(15), TaskCore.CachePolicy.NEVER_CACHE, c->TaskCore.Availability.present(), c->{
            GameThreadResult<MinecraftInstanceSnapshot> capture = ForgeGameThreadBridgeService.bridge().dispatch(new GameThreadRequest<>("scan-instance-capture", "Capture immutable scan input", GameThreadTarget.SERVER_MAIN_THREAD_CAPTURE, Duration.ofSeconds(3), c.cancellation(), () -> MinecraftInstanceService.current(false))).get(3, java.util.concurrent.TimeUnit.SECONDS);
            if (capture.status() != GameThreadBridgeStatus.COMPLETED) throw new IllegalStateException("Immutable scan capture unavailable: " + capture.status());
            MinecraftInstanceSnapshot immutable = capture.value().orElseThrow();
            GradleMcScanV1Writer.Result result = new GradleMcScanV1Writer().write(GradleMcPaths.gameDirectory(), immutable, new ArrayList<>(c.dependencies().values()));
            return output("scan:standard", "GradleMC Scan v1", "Wrote local bounded Scan v1 files.", capture(immutable), Map.of("scanId", result.scanId(), "location", "gradlemc/scans/" + result.scanId(), "atomicMoveFallback", Boolean.toString(result.atomicMoveFallback())));
        }));
    }
    private static void register(String raw, List<TaskId> dependencies, TaskCore.CachePolicy policy, String title, java.util.function.Function<InstanceSnapshot,Map<String,String>> fields) {
        TaskId id=TaskId.of(raw); REGISTRY.register(new TaskCore.Definition(id,title,"scan",dependencies,TaskCore.Affinity.SERVER_MAIN_THREAD_CAPTURE,Duration.ofSeconds(3),policy,c->TaskCore.Availability.present(),c->output(raw,title,"Collected from GradleMC's existing bounded local collector.",capture(),fields.apply(capture()))));
    }
    ...
    private static InstanceSnapshot capture() { return capture(MinecraftInstanceService.current(false)); }
    /** Converts an already-captured immutable collector value; safe to call from a worker. */
    private static InstanceSnapshot capture(MinecraftInstanceSnapshot old) {
        ...
        return new InstanceSnapshot(env.getOrDefault("gradlemc", GradleMC.CURRENT_VERSION), env.getOrDefault("minecraft", "unknown"), env.getOrDefault("loader", "Forge"), "unknown", runtime.getOrDefault("java", "unknown"), runtime.getOrDefault("vendor", "unknown"), "common", old.collectedAt(), 1, Map.of("count", Integer.toString(old.mods().value().mods().size())), old.resourcePacks().value().stream().map(p -> p.id()).toList(), List.of(), Map.of("count", Integer.toString(old.configs().value().size())), Map.of(), runtimeContext, stat, run);
    }
}
