...
public record GradleMCGuiState(
    ...
    private static final String MOD_VERSION = ModList.getModContainerById(GradleMC.MOD_ID)
            .map(container -> container.getModInfo().getVersion().toString())
            .orElse("");
    private static final String MINECRAFT_VERSION = GradleMC.CURRENT_MINECRAFT_VERSION;
    private static final String LOADER_VERSION = ForgeVersion.getVersion();

    public static GradleMCGuiState capture(SmartAIStatus status) {
        Minecraft minecraft = Minecraft.getInstance();
        String player = minecraft.player == null ? "" : minecraft.player.nameAndId().name();
